...
const productCellSearch = React.memo(
  ({
    data,
    index,
    didSelectAdd,
    ...
    let imageUrl = "";
    // if (data.media_gallery_entries && forSearch) {
    let image = data.media_gallery_entries.find(
      (entry) => (entry.types && entry.types.length) || entry.types
    );
    imageUrl = appMediaBaseUrl + `catalog/product/${image?.file}`;
    // }
    // else{
    let imageUrl1 = appMediaBaseUrl + data.image;
    // }
        ...
        >
          ...
          >
            ...
            >
              {/* <ImageLoader
                source={{
                  uri: forsearch ? imageUrl : imageUrl1,
                }}
                resizeMode={"contain"}
            ...
            >
              ...
              >
                <Text style={[styles.cost]}>
                  {Number(!forsearch ? data.finalPrice : data.price).toFixed(
                    3
                  ) +
                    " " +
                    currency}
...
export default productCellSearch;
